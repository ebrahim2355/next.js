...
Entries are matched against the request's `Origin` header, and only one part of it is used: the [`hostname`](https://developer.mozilla.org/en-US/docs/Web/API/URL/hostname). For a request from `http://local-origin.dev:3000/dashboard?tab=1`, only `local-origin.dev` is compared against your configuration, leaving out the scheme, the port, the path, and the query string. Write your entries that way too, without `https://` and without a port.
