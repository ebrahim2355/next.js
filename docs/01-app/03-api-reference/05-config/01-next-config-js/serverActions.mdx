...
Entries are matched against the request's `Origin` header, and only one part of it is used: the [`host`](https://developer.mozilla.org/en-US/docs/Web/API/URL/host), which is the hostname plus the port when the URL carries one. For a request from `https://my-proxy.com/checkout`, only `my-proxy.com` is compared against your configuration. For one from `https://my-proxy.com:8443/checkout`, only `my-proxy.com:8443`. Write your entries the same way.
